...
class TestPlaceEndpoints(unittest.TestCase):
    ...
    def test_place_amenities_relation(self):
        # On génère un nom unique pour éviter le conflit avec tes tests
        unique_amenity_name = f"WiFi_{uuid.uuid4().hex[:6]}"
        
        amenity_res = self.client.post('/api/v1/amenities/',
                                       json={"name": unique_amenity_name})
        
        # Sécurité pour débugger si ça échoue encore
        data = amenity_res.get_json()
        if 'id' not in data:
            raise Exception(f"Failed to create amenity: {data}")
